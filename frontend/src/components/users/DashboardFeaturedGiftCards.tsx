  ...
  return (
    <section className="container mx-auto my-16 px-4 max-w-7xl">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {giftCards.length > 0 ? (
            giftCards.map(card => (
              <div
                key={card._id}
                className="group bg-white/80 backdrop-blur-sm border border-blue-50 rounded-2xl shadow-lg hover:shadow-2xl hover:-translate-y-1 transition-transform p-6 flex flex-col items-center text-center"
              >
                {/* Image */}
                {card.imageUrl && (
                  <div className="w-20 h-20 mb-4 rounded-full bg-gray-100 p-3 shadow-md ring-2 ring-blue-100 group-hover:ring-blue-300 transition">
                    <img
                      src={card.imageUrl}
                      alt={card.brand}
                ...
                <div className="text-gray-600 text-sm mt-1 mb-4">
                  Value: <span className="font-semibold">${card.value}</span> | Price: <span className="text-green-600 font-semibold">${card.price}</span>
                </div>
      ...
      <div className="flex justify-center mt-10">
        <Link
          to="/shop"
          className="px-8 py-3 bg-gradient-to-r from-blue-600 to-fuchsia-500 text-white font-bold rounded-full shadow-md hover:shadow-lg hover:from-blue-700 hover:to-fuchsia-600 transition-all"
        >
          See More Gift Cards
        </Link>
      </div>
