...
export function AuthNavbar() {
  ...
  return (
    ...
    >
      <div className="flex justify-between items-center py-3 px-4">
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="relative" ref={dropdownRef}>
            <button className="focus:outline-none" onClick={() => setOpen(o => !o)} aria-label="User menu">
            ...
            {open && (
              <div className="absolute right-0 mt-3 w-56 bg-white/95 backdrop-blur-md border border-blue-100 rounded-xl shadow-lg py-2 z-30">
                ...
                <Link to="/profile" className="block px-4 py-2 hover:bg-blue-50 text-gray-700 text-sm transition" onClick={() => setOpen(false)}>
                  Profile
                </Link>
                <button onClick={handleLogout} className="block w-full text-left px-4 py-2 hover:bg-red-50 text-red-600 text-sm transition">
                  Logout
                </button>
              </div>
            )}
      ...
      {/* Mobile menu */}
      <div className="md:hidden border-t border-blue-50 bg-white/80 backdrop-blur-sm rounded-b-2xl">
        <div className="flex flex-col px-4 py-3 space-y-2">
          <Link to="/dashboard" className="text-gray-700 hover:text-blue-600 font-medium" onClick={() => setOpen(false)}>
            Dashboard
          </Link>
          <Link to="/shop" className="text-gray-700 hover:text-blue-600 font-medium" onClick={() => setOpen(false)}>
            Shop
          </Link>
        </div>
      </div>
